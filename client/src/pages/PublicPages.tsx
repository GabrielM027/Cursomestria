...
function SponsorsMarquee() {
  const items = buildSponsorTickerItems(sponsorSlots);
  return <div className="sponsors-marquee" aria-label="Espaços de patrocínio da pelada"><div className="club-container sponsors-marquee__inner"><div className="sponsors-marquee__label"><span>Apoio</span><strong>Patrocinadores</strong></div><div className="sponsors-marquee__viewport"><div className="sponsors-marquee__track">{items.map((sponsor, index) => <span className="sponsors-marquee__item" key={`${sponsor}-${index}`}>{sponsor}</span>)}</div></div></div></div>;
}
