...
export function HighlightsPage() {
  ...
  return <ClubShell><PageHero label="Personagens de cada domingo" title="Bola Cheia e Murcha">A última rodada em destaque e a classificação de quem mais se destacou na temporada.</PageHero><section className="section section--dark"><div className="club-container">{latestRound ? <section className="highlight-history__round"><div className="highlight-history__date"><span>Última rodada</span><strong>{formatDate(latestRound.date)}</strong></div><Highlights items={latestRound.items} /></section> : <Empty title="A história vai começar">Quando a primeira rodada tiver Bola Cheia e Bola Murcha lançadas, os destaques aparecem aqui.</Empty>}<section className="honor-rankings"><div className="section-heading honor-rankings__heading"><div><p className="section-label">Classificação dos destaques</p><h2 className="section-title">Quem está sendo <span>Bola Cheia e Murcha</span></h2></div><p className="section-copy">As vezes escolhidas e o total de votos recebidos por cada jogador na temporada.</p></div><div className="honor-rankings__grid"><section className="data-card"><RankingCardTitle label="Destaques positivos" title="Bola Cheia" /><RankingItems rows={data.honorRankings.best} kind="count" /></section><section className="data-card"><RankingCardTitle label="Destaques negativos" title="Bola Murcha" /><RankingItems rows={data.honorRankings.worst} kind="count" /></section></div></section>{previousRounds.length > 0 && <section className="highlight-history"><div className="section-heading highlight-history__heading"><div><p className="section-label">Rodadas anteriores</p><h2 className="section-title">Histórico por <span>domingo</span></h2></div></div>{previousRounds.map(round => <section className="highlight-history__round" key={round.date}><div className="highlight-history__date"><span>Domingo da rodada</span><strong>{formatDate(round.date)}</strong></div><Highlights items={round.items} /></section>)}</section>}</div></section></ClubShell>;
}
