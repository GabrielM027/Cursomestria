...
export function Home() {
  ...
  return <ClubShell>
    ...
    <section className="section section--dark"><div className="club-container"><div className="section-heading"><div><p className="section-label">Personagens da última rodada</p><h2 className="section-title">Melhor, pior e <span>resenha</span></h2></div><div className="section-heading__aside"><p className="section-copy">Somente os destaques do último domingo. O histórico completo fica na aba Melhor/Pior.</p><Link className="home-text-link" href="/destaques">Ver histórico <ChevronRight size={16} /></Link></div></div><Highlights items={latestRound} /></div></section>
...
export function HighlightsPage() {
  ...
  return <ClubShell><PageHero label="Personagens de cada domingo" title="Melhor e Pior">Cada rodada tem seus protagonistas. Aqui, a história fica organizada domingo por domingo.</PageHero><section className="section section--dark"><div className="club-container">{rounds.length ? <div className="highlight-history">{rounds.map(round => <section className="highlight-history__round" key={round.date}><div className="highlight-history__date"><span>Domingo da rodada</span><strong>{formatDate(round.date)}</strong></div><Highlights items={round.items} /></section>)}</div> : <Empty title="A história vai começar">Quando a primeira rodada tiver Melhor e Pior lançados, o histórico aparece aqui.</Empty>}</div></section></ClubShell>;
}
