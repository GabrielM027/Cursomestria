...
function copaStageLabel(stage: string) { return ({ quarterfinal: "Quartas de final", semifinal: "Semifinais", final: "Final", third_place: "3º lugar" } as Record<string, string>)[stage] || stage; }
...
function SelectionPlayer({ slot }: { slot: any }) {
  ...
  return <article className={`selection-slot selection-slot--${slot.role}-${slot.slot}`}><span className="selection-slot__role">{slot.label}</span>{player ? <><div className="selection-slot__avatar">{player.avatarUrl ? <img src={player.avatarUrl} alt={player.name} /> : <span>{player.name.slice(0, 1).toUpperCase()}</span>}{player.entityBadgeUrl && <img className="selection-slot__badge" src={player.entityBadgeUrl} alt="" />}</div><strong>{player.name}</strong><small>{player.votes} votos de Bola Cheia</small></> : <><div className="selection-slot__avatar selection-slot__avatar--empty"><CircleUserRound size={23} /></div><strong>A definir</strong><small>Aguardando votos</small></>}</article>;
}
...
export function SelectionOfYearPage() {
  ...
  return <ClubShell><PageHero label="Society · 7 na linha" title="Seleção do Ano">Os mais votados como Bola Cheia assumem suas posições no campo. A escalação atualiza automaticamente após cada partida lançada.</PageHero><section className="section section--dark"><div className="club-container"><div className="selection-year__intro"><div><p className="section-label">Escalação atual</p><h2 className="section-title">Os melhores no <span>campo</span></h2></div><p className="section-copy">Formação 1–2–2–2. O desempate considera mais vezes Bola Cheia e, depois, os pontos corridos.</p></div><div className="selection-field" aria-label="Campo society com a Seleção do Ano"><div className="selection-field__half"></div><div className="selection-field__circle"></div><div className="selection-field__goal selection-field__goal--top"></div><div className="selection-field__goal selection-field__goal--bottom"></div>{data.selectionOfYear.map((slot: any) => <SelectionPlayer key={`${slot.role}-${slot.slot}`} slot={slot} />)}</div><p className="selection-year__status">{filledSlots ? `${filledSlots} de 7 posições já têm líder por votos de Bola Cheia.` : "A seleção vai começar a se formar assim que os votos de Bola Cheia forem lançados."}</p></div></section></ClubShell>;
}
